...
def getFiles(folder, pattern, labelfile):
    ...
    for line in all_lines:
        ...
        # split every label. E.g first line:1000-IMG_MAX_116390 1000 is splited to
        splits = shlex.split(line)
        file_name = splits[0]  # 1000-IMG_MAX_116390
        class_id = splits[1]   # 1000
        ...
        #  remove all the endings

        # get now new file name
        true_file_name = os.path.join(folder, file_name + pattern)
        # file_name contain path, folder, name and new ending
        all_files.append(true_file_name)
        labels.append(class_id)
        # labels contain only number
...
def loadRandomDescriptors(files, max_descriptors):
    ...
    # 100 numbers are arranged out of order
    # Rearrange 100 labels in this order, this 100 is dimension D
    max_files = 100
    ...
    # tqdm is a progressbar for loops
    # open each of 100 files and loading its data to desc
    for i in tqdm(range(len(files))):
        ...
        # from array len(desc) random choice to form a new array, which size
        # is min(len(desc). The Probability to choice each element of array len(desc)
        # is max_descs_per_file.
    ...
    # concatenate all the arrays as a QxD matrix
    return descriptors
...
def assignments(descriptors, clusters):
    ...
    # use OpenCV to match the feature
    # TODO
    bf = cv2.BFMatcher()
    match = bf.knnMatch(descriptors, clusters, k=2)

    # create hard assignment
    assignment = np.zeros((len(descriptors), len(clusters)))  # TxK
    # TODO
    for i in range(len(descriptors)):
        assignment[i][match[i][0].trainIdx] = 1
...
def vlad(files, mus, powernorm, gmp=False, gamma=1000):
    ...
    parameters: 
        files: list of N files containing each T local descriptors of dimension N*D
        mus: KxD matrix of cluster centers                                      KxD
        gmp: if set to True use generalized max pooling instead of sum poolinggv
        encodings:NxK*D 
        desc:     T*D
        assignment:TxK
        f_enc:     D*K
    returns: NxK*D matrix of encodings 3600*64*100
    """
    K = mus.shape[0]
    encodings = []
    for f in tqdm(files):
        ...
        T, D = desc.shape
        dd = []
        f_enc = np.zeros((T, K*D))
        for k in range(mus.shape[0]):  # K 100
            ...
            index = np.nonzero(a[:, k])  # 获得非0元素的位置
            for j in index:
                f_enc[j, k*D:k*D+D] = desc[j] - mus[k]
        dd = np.sum(f_enc, axis=0).reshape(1, K*D)
        ...
        if powernorm:
            ...
            # get the sign
            sign = np.sign(dd)
            dd = sign * np.sqrt(np.abs(dd))
        # l2 normalization the weight vector (coef parameter of the SVM)
        # TODO
        dd = normalize(dd, norm='l2')  # 归一化
        if (len(encodings) == 0):
            encodings = dd
        else:
            encodings = np.vstack((encodings, dd))
            # 按垂直方向构建数组
...
def esvm(encs_test, encs_train, C=1000):
    ...
    encs_test as positive
    parameters: 
        encs_test: NxD matrix
        encs_train: MxD matrix

    returns: new encs_test matrix (NxD): new global descriptor
    """
    # set up labels
    # TODO
    M = encs_train.shape[0]
    N = len(encs_test)
    P = encs_train.shape[1]
    y = np.ones(M+1)
    weight = np.zeros((N, P))
    y[1:] = -1
    for i in range(N):
        X = np.append((encs_test[i].reshape(1,-1)), encs_train, axis=0)  # training data
        lin_clf = LinearSVC(penalty='l2', loss='squared_hinge', dual=True, tol=0.0001, C=1000)
        lin_clf.fit(X, y)
        weight_vector = lin_clf.coef_
            # coef_ has shape [n_class-1, n_features]
            #l2 normalize the weight vector
            #new global descriptor
...
def distances(encs):
    ...
    # encs = normalize(encs)
    dists = np.zeros((encs.shape[0], encs.shape[0]))
    for i in range(encs.shape[0]):
        for j in range(encs.shape[0]):
            dists[i][j] = 1 - np.dot(encs[i].reshape((1, -1)), encs[j].reshape((-1, 1)))
    np.fill_diagonal(dists, np.finfo(dists.dtype).max)
    # The result of Dot product of two same vector is 1, to avoid 0 in dists, we fill
    # elements on the diagonal as max. value in the dists
    return dists
...
if __name__ == '__main__':
    ...
    args.__setattr__('in_train', 'D:/PycharmProjects/pythonProject/exercise 3/train')
    args.__setattr__('in_test', 'D:/PycharmProjects/pythonProject/exercise 3/test')
    np.random.seed(42)
    gamma = 1000
    # a) dictionary
    ...
    if not os.path.exists('mus.pkl.gz'):
        # TODO descriptors
        descriptors = loadRandomDescriptors(files_train, max_descriptors=3000)
    ...
    if not os.path.exists(fname) or args.overwrite:
        # TODO
        enc_test = vlad(files_test, mus, True, gamma=1000)
    ...
    if not os.path.exists(fname) or args.overwrite:
        # TODO
        enc_train = vlad(files_train, mus, True, gamma=100)
        with gzip.open(fname, 'wb') as fOut:
    ...
    evaluate(enc_test, labels_test)
    print('> evaluate')
#Top-1 accuracy: 0.8194444444444444 - mAP: 0.6204548649445093
#Top-1 accuracy: 0.8863888888888889 - mAP: 0.7432624847312514